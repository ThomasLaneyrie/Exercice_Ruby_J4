def length(arr)
  puts "Nombre de handle dans cette fucking list : #{arr.length}"
end

def shorter(arr)    #A améliorer car me sort la 1ere valeur de l'array avec le nom le plus court, mais pas plusieurs
  puts "Voici le handle le plus court de la liste : #{arr.min {|a, b| a.length <=> b.length}}\n"
end
...
def alphabetic_sort (arr)
  print "Voici la liste par ordre alphabétique :\n #{arr.sort_by!(&:downcase)}"
end

def length_sort(arr)
  # puts arr.sort_by!(&:length) FONCTIONNE EGALEMENT
  print "Voici la liste triée par taille de handle :\n #{arr.sort!{|x, y| x.length <=> y.length}}"
end

def find_someone(arr,name)
  puts "Voici la position de #{name} dans la liste : #{arr.find_index(name)}"
end
...
arr_choix_question = ["- Combien il y a de Handle dans cette array ? Tapez 0", "- Quel est le handle le plus court de cette liste ? Tapez 1", "- Combien y-a t-il de handle contenant 5 caractères ? Tapez 2", "- Combien de handle commencent par une majuscule ? Tapez 3", "- Voulez-vous que je trie la liste des handle par ordre alphabétique ? Tapez 4", "- Voulez-vous que je trie la liste des handle par taille de handle ? Tapez 5", "- Voulez-vous connaitre la position de la personne @epenser dans la liste ? Tapez 6","- Voulez-voir voir une répartition des handle en fonction de leur taille ? Tapez 7"]
puts "\nA l'ancienne, tapez votre choix en fonction de ce que vous voulez (On se croirait aux votes de la star Academy hein) : \n"
